<?php
 ...
 if (mysqli_num_rows(mysqli_query($conn, "SELECT * FROM users1 WHERE email='{$email}'")) >
0) {
 $msg = "<div class='alert alert-danger'>{$email} - This email address has been already
exists.</div>";
 } else {
 if ($password === $confirm_password) {
 $sql = "INSERT INTO users1 (name, email, password, code) VALUES ('{$name}', '{$email}',
'{$password}', '{$code}')";
...
`SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`
 ...
 $mail->Body = 'Here is the verification link <b><a
href="http://localhost/login1/?verification='.$code.'"> Link </a></b>';
 $mail->send();
